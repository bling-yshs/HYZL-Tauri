...
use crate::entity::path::{SIGN_API_DIR, YUNZAI_DIR};
...
pub async fn start_yunzai_and_api() -> String {
    if !SIGN_API_DIR.exists() {
        return ReturnData::fast_failure("签名API文件夹不存在");
    }
    tokio::spawn(async {
        run_command(&path::SIGN_API_DIR,"start.bat",WindowType::WithWindowKeepOpen);
    });

    tokio::spawn(async {
        run_command(&path::YUNZAI_DIR,"node app",WindowType::WithWindowKeepOpen);
    });
...
pub async fn download_sign_api() -> String {
    ...
    let output = run_command(&path::PROGRAM_DIR, "git clone --depth 1 https://github.com/bling-yshs/unidbg-fetch-qsign", WindowType::Default);
...
#[test]
fn t() {
    let output = run_command(&path::PROGRAM_DIR, "chdidd", WindowType::Default);
